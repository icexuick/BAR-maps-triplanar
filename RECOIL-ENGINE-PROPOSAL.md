...
| **11** | **Industry techniques** (height-blend, stochastic, RVT cache, Tier-1 VRS, sediment, …) | Survey of modern AAA terrain tricks evaluated for top-down RTS, four of them live-tested in the prototype. RVT cache + Tier-1 VRS together can make the steady-state cost *cheaper* than Recoil today despite the richer shader. | See §11 |
...
### What ships in the prototype today

To set expectations: the [prototype](triplanar-terrain.html) implements §2-§7 in full, plus four §11.1 industry techniques as live toggles for direct A/B comparison:

- **Height-based blending** — global slider (Shader → Height Blend)
- **Squared triplanar weights** (Primozic) — toggle (HUD → SHARPEN BLEND)
- **Hex-tile / stochastic texturing** (Mikkelsen) — per-material toggle (Materials panel → Hex Tile)
- **Sediment / talus mask** — CPU-baked, ships in damage mask B channel (Sediment panel)

Not in the prototype (would need engine-side architecture): §11.1 RVT cache, Tier-1 VRS, multi-scale tiling, specular AA, micro-shadow, Texture2DArray bind. Those are recommended for the engine port but cannot be meaningfully demonstrated in a single-file three.js sandbox.
...
The **Status** column flags whether the technique was implemented and live-tested in the [prototype](triplanar-terrain.html) (✅), or remains a recommendation that needs engine-side architecture (📋).

| Technique | What | Why for RTS | Cost | Status | Source |
|---|---|---|---|---|---|
| **Height-based blending** | Replace smoothstep layer transitions with `max(h1+w1, h2+w2)` — layer's own height channel decides who wins per-pixel. Crisp interlocking transitions instead of muddy gradients. | Top of the list — biggest quality-per-byte win. Solves the "blurry seam" problem at zero per-pixel sampler cost. Composes perfectly with the §3 layer system. | +1 channel per layer (BC4 height map), a couple of ALU. **No extra taps.** | ✅ Prototype: live "Height Blend" slider. The prototype uses the sampled albedo's luma as an implicit per-texel height (no dedicated height maps needed) — the proper engine implementation should add a dedicated BC4 channel per material for predictable results. | [Frostbite SIGGRAPH 2007](https://media.contentapi.ea.com/content/dam/eacom/frostbite/files/chapter5-andersson-terrain-rendering-in-frostbite.pdf) |
| **Stochastic / hex-tile texturing** | Sample 3 randomly rotated/offset copies of a texture and blend with histogram preservation (Heitz/Deliot) or Mikkelsen's contrast-ramp simplification. Kills visible repetition on dominant ground textures. | RTS players see tiling more than anyone — fixed camera, slow zoom, wide-open ground. The biggest visible upgrade for the typical BAR view. Apply only to the dominant near-camera layer; far layers stay linear. | 3× taps per layer it's enabled on. Mitigate by gating to 1-2 layers and using detail-only sampling, or by caching via RVT (below). | ✅ Prototype: per-material "Hex Tile" toggle (Mikkelsen variant, biplanar-aware). Field test verdict: visual win on a single isolated layer is real but **modest in a multi-layer BAR-style stack** — once 4 layers compose, tiling artifacts are already largely hidden by the natural mask seams. Cost (3× taps on the opted-in material) ends up high relative to the visible payoff. Worth keeping as an opt-in for unusual single-material maps; not recommended as default-on. | [Heitz/Deliot](https://eheitzresearch.wordpress.com/738-2/), [Mikkelsen hex-tile](https://godotshaders.com/shader/stochastic-hex-tiling-mikkelsens-adaptation/) |
| **Runtime Virtual Texture (RVT) cache** | Bake the fully-composited terrain shading into a virtual texture, sample cheaply each frame. Re-tile only damaged/dirty regions. The damage mask from §4 already gives us the invalidation signal for free. | **Top performance recommendation if any one item is adopted.** Slow-moving RTS camera + heavy multi-layer shader + already-existing damage-mask invalidation = textbook RVT use case. Makes the high tap count from §10 almost irrelevant for steady-state frames. | Heavy initial bake on map load + per-blast sub-region re-bake. Near-free per-pixel after that. | 📋 Engine-only. Cannot be meaningfully demonstrated in a single-file three.js sandbox. | [Far Cry 4 AVT (GDC 2014)](https://gdcvault.com/play/1021761/Adaptive-Virtual-Texture-Rendering-in), [Unreal RVT](https://dev.epicgames.com/documentation/en-us/unreal-engine/runtime-virtual-texturing-in-unreal-engine) |
| **Tier-1 Variable Rate Shading (terrain only)** | Render terrain at 2×2 shading rate; units, props, decals at 1×1. The terrain shader is high-cost-low-frequency — the 2×2 quad rate is invisible from the RTS camera but cuts the work by 4×. Firaxis reported ~20% frame win in their RTS using exactly this split. | Free perf in the most expensive pass. Composes with everything else here. Requires Tier-1 VRS hardware (broadly available since 2019). | Negligible setup. | 📋 Engine-only (WebGL has no VRS). | [Firaxis VRS case study](https://devblogs.microsoft.com/directx/variable-rate-shading-a-scalpel-in-a-world-of-sledgehammers/) |
| **Snow/dust accumulation as a "fixed" top layer** | Slope + up-vector test (`saturate(dot(N, up) * snowAmt - heightFade)`) drives a top layer that accumulates on flat upward-facing surfaces. Already fits the §5 "fixed" layer concept — accumulation reads the original normal so blast craters don't auto-fill with snow. | Trivial to add given §3 + §5 are already specified. Big art payoff (winter maps, dust storms, ash). Map authors get it as a single layer declaration. | Negligible — math only on top of an existing layer slot. | 📋 Tested as a global feature in the prototype but reverted: the standard auto-layer system with a `snow` material + slope/height bands + `fixed` flag already covers this case more flexibly. The technique only earns its keep as a *separate* global feature when there's no general layer system — Recoil, post-§3, doesn't need it. | [Snowdrop engine](https://en.wikipedia.org/wiki/Snowdrop_(game_engine)) |
| **Sediment / talus mask** | CPU-bake a per-pixel "high neighbours above me" mask from the heightmap (8 ring samples, threshold + slope-fade). Drives a sediment material layer or a paint-mask channel. Naturally accumulates at the foot of cliffs and steep slopes. | Genuine eye-candy that **emerges from the heightmap itself** — no painting required. Fits BAR's typical large-cliff terrain perfectly. | CPU bake on map load. In main render: 0 extra taps if packed into a free channel (the prototype reuses damage mask B). | ✅ Prototype: live "Sediment" slider group, baked into damage mask B channel so it shares a sampler with scorch + crater depth. Toggle Debug Tint to visualise. Verdict: works well, very BAR-appropriate. | New (this proposal) |
| **Multi-scale detail tiling + detail normal fade** | Sample each layer at 2 frequencies (macro + meso) and modulate; cheaply hides repetition without going full stochastic. Detail normal at high frequency fades out by mip/distance to kill near-camera flatness when the user zooms in. | Cheap alternative or complement to stochastic. The fade-by-mip part costs nothing and visibly helps the close-zoom case BAR has. | +1 cheap macro tap per layer (low-res), or just a fade — trivial. | 📋 Not yet prototyped. | [iq texture repetition](https://iquilezles.org/articles/texturerepetition/) |
| **Specular AA (Toksvig / LEAN-style normal filtering)** | When mip-chaining many composited normals, specular aliases hard. Roughness-from-normal-variance (rough² += normal_variance) is the standard fix. | Becomes necessary once §3 layers stack normals from 4+ materials. Without it, distant terrain shimmers. | Negligible (roughness adjustment in the mip chain). | 📋 Needs engine-side mip-chain authoring; not feasible in a runtime three.js prototype. | [bgolus blending in detail](https://blog.selfshadow.com/publications/blending-in-detail/) |
| **Micro-shadow from layer height** | Cheap horizon-based contact shadow using each layer's own height channel (Oat/Steed style). Huge perceived contact detail at near-zero cost. | Free quality, obvious win once §3 layers carry height maps for height-blending anyway. | 1 ALU. | 📋 Tested in the prototype using screen-space luma derivatives as a fake height source — produced pixel-quad noise because composited albedo luma jumps hard between texels. Reverted. The proper implementation needs **dedicated per-material height maps** (which the engine port should add anyway for proper height-based blending), then this becomes the natural follow-up. | [bgolus blending in detail](https://blog.selfshadow.com/publications/blending-in-detail/) |
| **Texture2DArray per material slot** | Bind one array texture per channel-role (one for albedos, one for normals, one for height/AO/rough packs) and index by paint-mask channel or layer ID. | Effectively required for >4 distinct layers without bind-slot explosion. Lets the §3.2 paint mask address layers by index instead of name. | Memory-equivalent to separate textures; **better** cache behaviour. | 📋 Engine-only (WebGL1 has limited TextureArray support; would need WebGL2 + significant refactor). The prototype already runs into the 16-sampler cap — see §10 — exactly the problem this technique solves. | — |
| **Squared triplanar weights (Primozic)** | One-line tweak: square and renormalize the triplanar/biplanar blend weights. Sharper transitions on cliffs. | One line of GLSL, visible improvement. | Free. | ✅ Prototype: "SHARPEN BLEND" toggle in the HUD. Visibly cleaner cliff transitions. The prototype also adds a post-pow `1e-3` floor on the blend weights — necessary fix to prevent acute terrain corners from collapsing to near-black when both major and medium projection weights crash below the divide epsilon. | [primozic](https://cprimozic.net/notes/posts/a-small-change-to-improve-triplanar-mapping/) |
